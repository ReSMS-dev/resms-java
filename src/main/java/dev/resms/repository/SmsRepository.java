        ...
        if (httpResponse.statusCode() == HttpStatus.SC_OK) {
            ...
            try {
                ...
                switch (errorName) {
                    ...
                    case "COUNTRY_DETECTION_FAILED" -> throw new CountryDetectionFailedException(errorMessage);
                    case "PHONE_NUMBER_PARSING_FAILED" -> throw new PhoneNumberParsingFailedException(errorMessage);
                    case "INSUFFICIENT_SMS_QUOTA" -> throw new InsufficientSmsQuotaException(errorMessage);
                    case "MESSAGE_STATUS_UPDATE_FAILED" -> throw new MessageStatusUpdateFailedException(errorMessage);
